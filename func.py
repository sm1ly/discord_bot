...
import asyncio
import disnake
from database import create_tables_if_not_exist, get_user_data, get_user_data_by_static_id, save_user_data, \
    save_user_data_by_static_id, drop_table
...
async def menu(uid, message):
    if uid not in user_data:
        ...
        buttons = [
            discord.ui.Button(label="Организация транспортировки | 2 Монеты", custom_id="button_1",
                              style=discord.ButtonStyle.blurple),
            discord.ui.Button(label="Услуги консьержа 24/7 | 1 монета", custom_id="button_2",
                              style=discord.ButtonStyle.blurple),
            discord.ui.Button(label="Юридическая помощь | 1 монета", custom_id="button_3",
                              style=discord.ButtonStyle.green),
            discord.ui.Button(label="Услуги аренды спортивного инвентаря | Индивидуально", custom_id="button_4",
                              style=discord.ButtonStyle.blurple, disabled=True),
            discord.ui.Button(label="Сервис удаления персональных данных | 10 монет", custom_id="button_5",
                              style=discord.ButtonStyle.green, disabled=True),
            discord.ui.Button(label="Эксклюзивные вечеринки | 10 монет", custom_id="button_6",
                              style=discord.ButtonStyle.red, disabled=True),
            discord.ui.Button(label="Специальные мероприятия | 10 монет", custom_id="button_7",
                              style=discord.ButtonStyle.red, disabled=True)
        ]
        buttons_vip = [
            discord.ui.Button(label="Организация транспортировки | 2 Монеты", custom_id="button_1",
        ...
        try:
            interaction = await client.wait_for("button_click", timeout=30.0, check=lambda
                i: i.message.id == msg.id and i.user.id == message.author.id)
...
async def check_balance(uid, author, message):
    ...
    await author.send(f"{author.display_name}, Ваш баланс: {balance} монет.")
...
async def paid_check(uid, author, message):
    static_id = author.display_name.split("|")[1].strip() if "|" in author.display_name else None
    if static_id is None:
        ...
        await message.delete()
        await message.author.send(f"{author.display_name}, вы уже получили свою монету!")
        ...
        await message.author.add_roles(disnake.utils.get(server.roles, id=config.ROLE_ID_Guest))
        # Добавляем роль "MODERATE" пользователю
        await message.author.add_roles(disnake.utils.get(server.roles, id=config.ROLE_ID_MODERATE))

        # await message.channel.set_permissions(message.author, send_messages=False)
        await message.delete()
        await message.author.send(
            f"{author.display_name}, Вы получили свою монету и стали нашим Гостем. После проверки Вам будет доступен новый функционал!")
...
async def paid_vip_check(uid, author, message):
    ...
    if user_data[uid]["coins"] >= 10:
        ...
        role = disnake.utils.get(server.roles, id=config.ROLE_ID_VIP)
        if role is not None:
            await message.author.add_roles(role)
        await message.channel.set_permissions(message.author, send_messages=False)
        await message.delete()
        ...
        await save_user_data(user_data)
        await message.author.send(
            f"{author.display_name}, Вы потратили 10 монет! Вы стали VIP! Теперь для Вас доступен новый функционал!")
        await message.author.send(f"Подробнее в <#{config.CHANNEL_ID_VIP_SERVICE_COST}>")
    else:
        await message.delete()
        await message.author.send(f"{author.display_name}, Кажется Вы не внесли взнос в размере 10 монет!")
...
async def send_moderation_message(uid, static_id):
    ...
    message = await channel.send("Вы хотите продолжить?", components=[[yes_button, no_button]])

    # Ждем ответа от пользователя
    try:
        interaction = await client.wait_for(event=disnake.InteractionType.component, check=lambda inter: inter.message.id == message.id, timeout=30.0)
    except disnake.InteractionTimedOut:
        await message.edit(content="Вы не ответили вовремя.")
        return
    ...
    await interaction.response.send_message(f"Вы нажали кнопку {response}.")
